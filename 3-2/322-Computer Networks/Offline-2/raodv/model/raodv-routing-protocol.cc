...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        if(flag==0)SendReply(rreqHeader, toOrigin);
        else SendRevRequest(rreqHeader,toOrigin);
        return;
    }
    /*
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if ((rreqHeader.GetUnknownSeqno() ||
            ...
            toDst.GetValidSeqNo())
        {
            if (!rreqHeader.GetDestinationOnly() && toDst.GetFlag() == VALID && flag==0)
            {
                m_routingTable.LookupRoute(origin, toOrigin);
...
RoutingProtocol::SendRevRequest(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    //myedit
    RevRreqHeader revreqHeader(/*m_flags=*/0,
                            /*m_reserved=*/0,
                            /*m_hopCount=*/0,
                            /*m_requestID=*/m_requestId,               //rreqHeader.GetId(),
                            /*m_dst=*/rreqHeader.GetDst(),
                            /*m_dstSeqNo=*/m_seqNo,
                            /*m_origin=*/rreqHeader.GetOrigin(),
                            /*m_originSeqNo=*/rreqHeader.GetOriginSeqno());

    // RevRrepHeader rrepHeader(/*prefixSize=*/0,
    //                       /*hopCount=*/0,
    //                       /*dst=*/revrreqHeader.GetDst(),
    //                       /*dstSeqNo=*/m_seqNo,
    //                       /*origin=*/toOrigin.GetDestination(),
    //                       /*lifetime=*/m_myRouteTimeout);
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        revreqHeader.SetOrigin(iface.GetLocal());
        m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);

        Ptr<Packet> packet = Create<Packet>();
        SocketIpTtlTag tag;
        //tag.SetTtl(ttl);
        tag.SetTtl(m_ttlStart);
        packet->AddPacketTag(tag);
        ...
        NS_LOG_DEBUG("Send REVRREQ with id " << revreqHeader.GetId() << " to socket");
        m_lastBcastTime = Simulator::Now();
        Simulator::Schedule(Time(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10))),
                            &RoutingProtocol::SendTo,
                            this,
...
RoutingProtocol::RecvRevRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    // If RREP is Hello message
     if (dst == rrepHeader.GetOrigin())  //lagbena
    {
        //ProcessHello(rrepHeader, receiver);
        return;
    }

    uint32_t id = rrepHeader.GetId();
    ...
    RoutingTableEntry newEntry(
        ...
        /*nextHop=*/sender,
        /*lifetime=*/m_myRouteTimeout);
        //Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime));   //revreq has no lifetime
    RoutingTableEntry toDst;
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        //newEntry.SetLifeTime(std::max(Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime), toDst.GetLifeTime()));
    ...
    RoutingTableEntry toOrigin;
    if (!m_routingTable.LookupRoute(rrepHeader.GetDst(), toOrigin) ||
        toOrigin.GetFlag() == IN_SEARCH)
    {
        return; // Impossible! drop.
    }
